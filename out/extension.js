...
let aiInsertStartLine = null;
let aiInsertDocument = null;
...
function activate(context) {
    ...
    const toggleCommand = vscode.commands.registerCommand("aiAnnotator.toggle", () => {
        extensionEnabled = !extensionEnabled;
        vscode.window.showInformationMessage(extensionEnabled
    ...
    vscode.workspace.onDidChangeTextDocument(async (event) => {
        ...
        if (inserted.includes("AI_ASSISTED") ||
        ...
        if (lines.length < 2 && inserted.length < 25)
            return;
        ...
        debounceTimer = setTimeout(async () => {
            if (!aiInsertDocument || aiInsertStartLine === null)
                return;
...
async function annotateBlock(document, startLine, empId) {
    const editor = vscode.window.activeTextEditor;
    if (!editor)
        return;
    isAnnotating = true;
    const startTag = `// AI_ASSISTED: true | DEVELOPERS: ${empId}\n`;
    const endTag = `\n// AI_ASSISTED_END\n`;
...
function updatePercentage(document) {
    ...
    const percent = total === 0 ? 0 : Math.round((ai / total) * 100);
