...
async function openReportPanel() {
    ...
    panel = vscode.window.createWebviewPanel('aiDashboard', 'AI Annotator — Enterprise Dashboard', vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
    ...
    panel.webview.onDidReceiveMessage(async (msg) => {
        switch (msg.command) {
            ...
            case 'exportCSV':
                ...
                break;
            case 'approveBlock':
                (0, approvalService_1.approveBlock)(msg.file, msg.hash, msg.date, (0, employeeService_1.getEmployeeId)());
                await refreshPanel();
                break;
            case 'rejectBlock':
                (0, approvalService_1.rejectBlock)(msg.file, msg.hash, msg.date, (0, employeeService_1.getEmployeeId)());
                await refreshPanel();
                break;
        }
    });
...
async function refreshPanel() {
    ...
    const violations = (0, policyService_1.evaluatePolicy)(blocks);
    const auditEntries = (0, auditLogService_1.readAuditLog)().slice(-100);
    const chainResult = (0, auditLogService_1.verifyChain)();
    const approvals = (0, approvalService_1.getAllApprovals)();
    const caps = (0, rbacService_1.getDashboardCapabilities)();
    panel.webview.html = generateHTML(blocks, snapshots, teamConfig, threshold, extensions, totalAI, totalAll, violations, auditEntries, chainResult, approvals, caps, config);
}
// ── CSV Export ────────────────────────────────────────────────────────────────
async function exportCSV(blocks) {
    const header = 'Employee,Date,File,Lines,Status,Reviewer,EditedBy,Hash,LicenseRisk\n';
    const rows = blocks.map((b) => `${b.employeeId},${b.date},"${b.file}",${b.lines},${b.status || 'PENDING'},${b.reviewer || ''},"${b.editedBy.join(' | ')}",${b.hash || ''},${b.licenseRisk?.license || ''}`).join('\n');
...
// ── HTML Generation ───────────────────────────────────────────────────────────
function generateHTML(blocks, snapshots, teamConfig, threshold, extensions, totalAI, totalAll, violations, auditEntries, chainResult, _approvals, caps, config) {
    // ── Aggregations ──────────────────────────────────────────────────────────
    ...
    // Tab visibility
    const visibleTabs = caps.visibleTabs;
...
<nav>
  ...
  <div class="nav-meta">
    <span class="info-chip">👤 ${(0, employeeService_1.getEmployeeId)()}</span>
